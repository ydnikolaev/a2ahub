...
var foldedFieldsItemDeliberatelyDrops = map[string]string{
	...
	"ContractNonAdoptable":      "F4 (agent-exchange-2026-08 wave 36): the descriptor's own x_binding refusal of `a2a contract adopt`, decoded once in mirror.go's build pass (contractNonAdoptableFromRaw) alongside OperationalItems. Same category as LatestPublishVersion just above: a contract-only fact consumed by the contracts catalog (cache.ContractInfo -> html.Contract), not by a generic inbox/outbox item row",
}
