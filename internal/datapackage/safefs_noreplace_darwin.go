...
// were. RENAME_EXCL is what makes "or" true: the kernel refuses rather than
// clobbering an existing destination, so a fetch racing another writer can
// never half-replace a directory a reader is already using.
//
// Mirrors internal/space/contract_materialize_noreplace_darwin.go. The two are
