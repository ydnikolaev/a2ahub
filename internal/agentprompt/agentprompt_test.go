...
func TestAgentPromptKeepsOnlyOurOwnMoves(t *testing.T) {
	...
	got := Of(actions, string(fold.KindQuestion), "checkout", false)
	if got == nil {
	...
	if Of(actions, string(fold.KindQuestion), "ledger", false) != nil {
...
func TestPromptPointsAtWhatGetsDrafted(t *testing.T) {
	...
	for _, tc := range cases {
		got := Of([]Action{{Transition: tc.move, By: []string{"checkout"}}}, string(tc.subject), "checkout", false)
		if got == nil {
