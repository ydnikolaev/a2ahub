...
func Assemble(ctx context.Context, store *cache.Store, self string, now time.Time) (Data, error) {
	...
	d.Unavailable = append(d.Unavailable,
		...
		UnavailableFact{ID: "artifact-detail-bodies", Scope: "artifacts", Title: "Canonical artifact detail bodies", Reason: "The current live dashboard assembler exposes thread evidence but does not mount the bounded show/detail projection yet.", SourceClass: "unavailable"},
...
func toItem(it cache.Item, now time.Time) Item {
	...
	return Item{
		...
		SyncStale: it.SyncStale, Description: it.Description,
	}
}
