...
func TestImportBoundaryMatchesADR001(t *testing.T) {
	...
	// hand-copied os.Stat+t.Skipf pair, byte-identical to
	// internal/localserver's own — a `_test.go` helper cannot be imported
	// across packages, so the duplication was forced by Go, and the fix is
	// this one shared, importable helper.
