...
func TestRealCatalogueDeclaresEveryTierCoherently(t *testing.T) {
	...
	//
	// It did NOT become 36/14 on 2026-08-21, and the near-miss is worth
	// keeping: judge-the-thing P9's declared-companion row was applied here
	// with its counters and WITHOUT a driver, and TestLogicMatrix reported
	// `not-run=1`. A catalogue row with no scenario behind it is a claim of
	// coverage that does not exist — the exact defect that epic is named
	// after, introduced into the file that exists to prevent it. Reverted;
	// the row and its driver are one commit, never two.
	//
