...
- `contract publish` — publish a contract version (--version/--bump, --staging, --allow-empty-bump)
- `contract retire` — retire a contract (consumer-ack precondition, --override)
- `contract verify-export` — verify a local export's digest tree (--local, --json)
- `contract verify-published` — report whether every published contract this system provides still matches its code (--json, --local <id>=<path>)
...
- `docs` — print the embedded a2ahub documentation by topic (a2a docs [topic])
...
- `a2a_contract` — contract family: action=new|preflight|publish|materialize|check|deprecate|retire|diff|verify-export|verify-published|adopt|activate
- `a2a_data` — contract data exchange loop: action=pack|deliver|fetch|verify|attach|fetch-blob
- `a2a_docs` — embedded a2ahub documentation: omit topic to list every known topic (id/group/title), or pass one to read its page
