...
| A write was REFUSED with a code you do not recognize — `SCH-`, `POL-`, `REF-`, `LFC-`. This is not a loop either: read the code, fix the named thing, re-run the verb. | [reference/threads.md](reference/threads.md) for validation, format and classification refusals · [troubleshooting.md](troubleshooting.md) for `doctor` rows, MCP refusals and host/credential causes | — |
