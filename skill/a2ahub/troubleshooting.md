...
| **`a2a submit` refused with `space: attachment does not resolve through the space's own resolution path`** | Every draft `a2a attach` produces hits this today — see [loops.md](loops.md) §8.2 step 2. It is not something to retry or work around locally: do not use `a2a attach` to send bytes yet. If your draft needs to move a payload, the working path is the `work_request`/`category: data` → `a2a data pack`/`a2a data deliver` flow instead ([reference/data-exchange.md](reference/data-exchange.md)), which moves bytes only from a counterparty back to you. |
