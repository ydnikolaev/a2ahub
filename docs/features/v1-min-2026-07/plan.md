...
- Wave 7a — P13 catalog seam (partial P13) — 2026-07-22 — phase P13 (→ in-progress) — commits cc44b7b (plan+reconcile), ed43fb7 (code+reference+CI) — make check green — hidden `a2a __catalog` emits deterministic markdown (42 cmds + 32 MCP tools) read from cli.Command.Synopsis()/mcp.Registry.List(); reference/commands.md + 8 authoring/*.md generated byte-identical; skill-drift CI job wired; NO internal/* touched (§7/§Footprint stand); propagate probe → 3 doc-only drift items (3-row hand-typed caveat + resume-state --json→markdown) amended lead-inline; prose 7b deferred by operator — details: plans/13-agent-skill.plan.md
...
- **Code track — P13 (skill) DONE (waves 7a + 7b, 2026-07-22).** 7a shipped the
  ...
  kept for the seam-resolution record.
  **Catalog-seam fork RESOLVED + SHIPPED (wave 7a, 2026-07-22).** The design
  fork ("how to emit the catalog") was a lead technical call (advisor-confirmed:
  the operator had no basis to choose the mechanism). Resolution: a hidden
  `a2a __catalog` verb in `cmd/a2a` (NOT `--json` — it emits **markdown**, byte
  = `skill/a2ahub/reference/commands.md`) that builds the CLI half by
  constructing each `cli.Command` with nil/stub deps to read `Synopsis()` and
  the MCP half from `mcp.BuildRegistry(...).List()` — no `internal/*` change, no
  buildCommands restructure (see spec 13 §11 wave-7 amendment + plans/
  13-agent-skill.plan.md). Shipped: `catalog.go` + name-parity guard, 42 command
  + 32 MCP-tool rows, `reference/commands.md` + 8 `reference/authoring/*.md`
  generated from the binary. The `skill-drift` CI job is lead-wired (teeth
  proven both ways). **7b DONE (d80c8e4):** the hand-maintained prose
  (`SKILL.md`, `loops.md`, `troubleshooting.md`, `onboarding.md`,
  `decompose-example.md`, `RELEASE-CHECKLIST.md`) authored at opus + lead
  line-by-line verified. P13 complete.
