...
| `commit-sha-reference.md` | a 40-char hex string superficially resembles an API secret | it's a benign Git object reference in artifact prose; D-023 derives such SHAs from history rather than storing one in the publish event | budget: 0 — bare hex SHAs are common and legitimate; the scanner must not flag hex-only strings without a recognized credential prefix |
